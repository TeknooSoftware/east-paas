<?php

declare(strict_types=1);

/*
 * East Paas.
 *
 * LICENSE
 *
 * This source file is subject to the MIT license
 * it is available in LICENSE file at the root of this package
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to richard@teknoo.software so we can send you a copy immediately.
 *
 *
 * @copyright   Copyright (c) EIRL Richard Déloge (https://deloge.io - richard@deloge.io)
 * @copyright   Copyright (c) SASU Teknoo Software (https://teknoo.software - contact@teknoo.software)
 *
 * @link        http://teknoo.software/east/paas Project website
 *
 * @license     http://teknoo.software/license/mit         MIT License
 * @author      Richard Déloge <richard@teknoo.software>
 */

namespace Teknoo\East\Paas\Compilation\Compiler\Quota;

use Teknoo\East\Paas\Compilation\Compiler\Exception\QuotaWrongConfigurationException;
use Teknoo\East\Paas\Contracts\Compilation\Quota\AvailabilityInterface;

use function class_exists;
use function is_a;

/**
 * Factory to create a quota instance defined by its keys as constructor's parameters during a deployment in a JobUnit
 * or the QuotaCompiler
 *
 * @copyright   Copyright (c) EIRL Richard Déloge (https://deloge.io - richard@deloge.io)
 * @copyright   Copyright (c) SASU Teknoo Software (https://teknoo.software - contact@teknoo.software)
 * @license     http://teknoo.software/license/mit         MIT License
 * @author      Richard Déloge <richard@teknoo.software>
 */
class Factory
{
    /**
     * @param array<string, class-string> $configuration
     */
    public function __construct(
        private array $configuration,
    ) {
    }

    public function create(string $category, string $type, string $capacity, bool $isSoft): AvailabilityInterface
    {
        if (!isset($this->configuration[$category])) {
            throw new QuotaWrongConfigurationException(
                message: "The resource availability category `{$category}` does not exist",
                code: 500,
            );
        }

        $class = $this->configuration[$category];

        if (
            !class_exists($class, true)
            || !is_a($class, AvailabilityInterface::class, true)
        ) {
            throw new QuotaWrongConfigurationException(
                message: "The resource category class `{$this->configuration[$category]}` does not exist",
                code: 500,
            );
        }

        return new $class(
            type: $type,
            capacity: $capacity,
            isSoft: $isSoft,
        );
    }
}
