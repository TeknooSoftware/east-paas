Feature: Execute a job to deploy a project on a cluster
  ...
  Scenario: From the API, run a job on a project using prefix with a PaaS file using conditions and get a normalized
  job's history
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project using prefix with a PaaS file using conditions, on encrypted messages
  ...
  Scenario: From the API, run a job on a project using prefix with a PaaS file using jobs and get a normalized job's
  history
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project using prefix with a PaaS file using jobs, on encrypted messages between
