Feature: Execute a job to deploy a project on a Kubernetes cluster
  ...
  using image volumes and hostUsers false
    Given I have a configured platform
  ...
  using image volumes but without hostUsers
    Given I have a configured platform
  ...
  Scenario: From the API, for a Kubernetes 1.35 cluster, run a job on a project with a PaaS file using jobs and get
  manifests using image volumes but without hostUsers
    Given I have a configured platform
    And the kubernetes cluster runs version "1.35"
    And the platform is booted
    And a project with a complete paas file with jobs
  ...
  Scenario: From the API, for a Kubernetes cluster, run a job on a project with a PaaS file using expose shortcuts (v1.2)
  and get a normalized job's history
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, for a Kubernetes cluster, run a job on a project with a PaaS file using expose shortcuts but
  file has wrong version and get an error
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, for a Kubernetes cluster, run a job on a project with a PaaS file using expose shortcuts and a service
  ...
  Scenario: From the API, for a Kubernetes cluster, run a job on a project with a PaaS file using expose shortcuts and an ingress
  duplicated by an explicit definition and get an error
    Given I have a configured platform
    And the platform is booted
