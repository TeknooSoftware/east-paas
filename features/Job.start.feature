Feature: Execute a job to deploy a project on a cluster
  ...
  Scenario: From the API, run a job on a project with a PaaS file using conditions but file has wrong version and get an
  error
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project with a PaaS file using jobs but the PaaS file has a wrong version and
  get an error
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project with a PaaS file using jobs and get a normalized job's history
    Given I have a configured platform
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project with a PaaS file using jobs, on encrypted messages between
  workers and get a normalized job's history
    Given I have a configured platform
    And encryption capacities between servers and agents
    And the platform is booted
  ...
  Scenario: From the API, run a job on a project with a PaaS file using expose shortcuts (v1.2) and get a normalized
  job's history
    Given I have a configured platform
    And the platform is booted
    And a project with a paas file using expose shortcuts
    And a job workspace agent
    And a git cloning agent
    And a composer hook as hook builder
    And an image builder
  ...
  Scenario: From the API, run a job on a project with a PaaS file using expose shortcuts but file has wrong version and
  get an error
    Given I have a configured platform
    And the platform is booted
    And a project with a paas file using expose shortcuts with wrong version
    And a job workspace agent
    And a git cloning agent
    And a composer hook as hook builder
    And an image builder
    ...
    When I run a job "jobid" from project "projectid" to "/project/projectid/environment/prod/job/jobid/run"
    Then I must obtain an HTTP answer with this status code equals to "400"
    And with this body answer, the problem json, 'expose shortcuts validation error'

  Scenario: From the API, run a job on a project with a PaaS file using expose shortcuts and a service
  duplicated by an explicit definition and get an error
    Given I have a configured platform
    And the platform is booted
    And a project with a paas file using expose shortcuts and a duplicated service
    And a job workspace agent
    And a git cloning agent
    And a composer hook as hook builder
    And an image builder
    ...
    When I run a job "jobid" from project "projectid" to "/project/projectid/environment/prod/job/jobid/run"
    Then I must obtain an HTTP answer with this status code equals to "400"
    And with this body answer, the problem json, 'duplicated service error'

  Scenario: From the API, run a job on a project with a PaaS file using expose shortcuts and an ingress
  duplicated by an explicit definition and get an error
    Given I have a configured platform
    And the platform is booted
    And a project with a paas file using expose shortcuts and a duplicated ingress
    And a job workspace agent
    And a git cloning agent
    And a composer hook as hook builder
    And an image builder
    ...
    When I run a job "jobid" from project "projectid" to "/project/projectid/environment/prod/job/jobid/run"
    Then I must obtain an HTTP answer with this status code equals to "400"
    And with this body answer, the problem json, 'duplicated ingress error'
